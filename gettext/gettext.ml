(* There is no language support for ocaml code in GNU xgettext when
   ...
   open Gettext, for example by the use of the form 'Gettext.(gettext
   "...")'.  Note that ocaml comments (such as this comment block) are
   not recognised as comments and therefore will be scanned, so avoid
   a gettext S-expression appearing in comments if text which is not
   in fact for translation is to be excluded from the pot file. *)
